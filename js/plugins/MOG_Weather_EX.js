...
/*:
 * @target MZ
 * @plugindesc 強化天氣效果 (v1.3) Adiciona novos efeitos de climas.
 * @author Moghunter
 ...
 * @option 疊加1 (耕作模式->"精靈循環")
 * @value Overlay 1
 * @option 疊加2 (精靈模式->較少延遲)
 * @value Overlay 2
 *
 * @arg power
 * @desc 定義粒子的數量。
 ...
 * 
 * 添加新的天氣效果。
 * 天氣圖像必須收錄在文件夾中。(img/weather/)
 * 
 * - MAP NOTETAGS
 * 要激活氣候，請通過PLUGIN COMMAND功能使用以下註釋。
 * 
 * weather ID : TYPE : POWER : SPEED : BLEND_TYPE : FILE_NAME 
 * -> ID - 定義氣候ID，可以同時使用多達10種不同的氣候。
 * -> TYPE - 由 0 ~ 29 組成。
 *       0-風1 (落下)
 *       1-風2 (左側)
 *       2-風3 (變焦)
 ...
 *      35-疊加1 (耕作模式->"精靈循環")
 *      36-疊加2 (精靈模式->較少延遲)
 * -> POWER - 氣候的力量 (顆粒數量) (1 ~ 2000)
 * -> SPEED - 速度-設置動畫的速度。 (0% ~ 1000%)
 * -> BLEND_TYPE - 混合類型 (0 ~ 1)
 * -> FILE_NAME - 圖片文件的名稱。
 * 
 * 示例：
 ...
 * -> Z-INDEX - 定義氣候將在圖塊集之上還是之下。
 *       0 - 低於圖塊集。
 *       1 - 在圖塊上方。
 * -> ZOOM - 增大或減小圖像尺寸。 (0% ~ 800%)  
 * 
 * 示例：
